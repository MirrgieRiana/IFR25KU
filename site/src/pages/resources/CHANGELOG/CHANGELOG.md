...
- 変更: スタッフの射撃をチャージ式に変更。
  - 使用中はチャージが進み、解除時に射撃する。
  - チャージが満ちる前に解除した場合は射撃しない。
  - チャージが満ちたときに音が鳴る。
  - チャージ中も歩行速度が低下しない。
  - チャージ時間は素材の採掘速度に反比例し、鉄と同じ採掘速度で2秒となる。
  - 魔法加速はチャージ時間を短縮する。
  - 射撃後のクールタイムを撤廃。
  - 説明文を「使用時、射撃攻撃」から「使用中、チャージし、解除時に射撃攻撃」に変更。
- 変更: スタッフの魔法射撃攻撃のダメージに素材のエンチャント適性による補正を追加。
  - 鉄と同じエンチャント適性を基準として、そこから1離れるごとに0.25増減する。
- 表示: スタッフの手持ち時のモデルを、杖を立てて構えた姿勢に変更。
