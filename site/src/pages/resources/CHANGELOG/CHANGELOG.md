...
- 変更: 特性「棘のある葉」の成長ブーストを0.2から0.5に上昇。
- 追加: 特性「食虫植物」に捕食ダメージを追加。
- 変更: 特性「食虫植物」の特性条件「日照環境」を削除。
- 変更: 特性「食虫植物」の栄養値を0.2から0.5に上昇。
...
- 変更: 琥珀色の原生林を多湿バイオームとして扱うようにした。
  - 魔法植物の「高湿度」特性条件が、このバイオームで成立するようになった。
  - 沼地と同様に、火の延焼が起こりにくくなった。
...
- 変更: 琥珀色の原生林にシダが生成されなくなった。
  - 大きなシダも生成されなくなった。
- 変更: 琥珀色の原生林の草の生成密度を減少。
- 変更: 琥珀色の原生林にキノコが生成されなくなった。
- 変更: 砂岩硝石鉱石の生成頻度を2倍に増加。
- 変更: 琥珀色の原生林でのスケルトンスポナーの生成頻度を4分の1に減少。
- 変更: 土妖精のプラスチック鉱石が、草ブロックやポドゾルなどではなく、土ブロックのみに生成されるようになった。
- 修正: 妖精の樹海・琥珀色の原生林・過去を見つめる都市の地表のブロックの分布をわずかに変更。

その他

- 修正: 妖精の工場がフォリアを回収する際に辿るブロックを、ブロックタグ `miragefairy2024:folia_conductors` で判定するようになった。
...
- 変更: 共通機械レシピのレシピビューアーの出力にレシピリマインダーも含めるようにした。
- 変更: ハイメヴィスカの涙の燃料値を精錬8回分に上昇。
...
- 変更: ステータス効果「経験値獲得」は、1秒間にレベルと同じ量の経験値を得るようになった。
  - 1秒をレベルで割り切れないレベルIIIなどでは、獲得の間隔がなるべく均等になるように配分される。
  - 効果時間が獲得の間隔で割り切れない場合、合計の獲得量は四捨五入される。
- 変更: ハイメヴィスカの樹液と妖精のリキュールの経験値獲得の効果時間を、それぞれ2秒と16秒に延長。
  - 経験値の合計獲得量は、どちらも変更前と変わらない。
- 変更: 石化した樹脂状の土およびその派生建材の音を、泥系から凝灰岩系に変更。
