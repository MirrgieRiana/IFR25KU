...
- 追加: ディメンション「マントル」を追加。
  - Y-64とY256に岩盤があり、その間はすべて岩石で埋まっている。
  - 洞窟や空洞は生成されない。
  - 地上世界との距離の係数は4倍。
- 追加: 地形生成物「フェアリークエストゲート」を追加。
  - 地上世界の地下の、最下層からY-32までに稀に生成。
  - 岩石に完全に埋もれた状態で生成。
  - ゲートに入るとマントルへ移動する。
  - 出口側にゲートが無い場合は、出口側にも同じゲートが生成される。
  - 地上世界からマントルへ入ると、Y64付近に出る。
- 追加: ブロック「フェアリークエストゲート」を追加。
  - フェアリークエストゲートの枠を構成する。
  - ネザーポータルと同じ形に枠を組んで火を点けると、内側にポータルが現れる。
  - 枠が欠けるとポータルは消える。
- 追加: ブロック「フェアリークエストゲートのポータル」を追加。
  - フェアリークエストゲートの内側を満たす。
- 追加: マントルを構成する岩石ブロックを5種類追加。
  - ブリッジマナイト、フェロペリクレース、マジョライト、アキモトアイト、ポストペロブスカイト。
  - マントル内での分布はノイズによって決まる。
- 追加: 鉱石ブロック「ワズレアイト鉱石」を追加。
  - マントルに生成。
- 追加: 鉱石ブロック「リングウッダイト鉱石」を追加。
  - マントルに生成。
- 追加: 素材アイテム「ワズレアイト」「リングウッダイト」を追加。
- 追加: 建材ブロック「強化金属ブロック」「強化金属タイル」「模様入りの強化金属」「妖精研究所の床」およびそれぞれのハーフブロックと階段を追加。

モブ

- 追加: モブ「マントルのフェアリーウィスプ」を追加。
  - マントルにのみ湧く。
  - 普段はプレイヤーの近くを漂うだけだが、攻撃すると反撃してくる。
  - 体力と攻撃力はピグリンブルートと同等。
  - 倒すとワズレアイトを落とす。

採掘

- 追加: ブロックタグ「Tier4のツールが必要」「Tier5のツールが必要」を追加。
  - Tier4のツールが必要なブロックは、ネザライト以上のつるはしでしか回収できない。
  - Tier5のツールが必要なブロックは、ネザライトより更に1段階上のつるはしでしか回収できない。
- 追加: マントルディメンションのブロックは、適正でないツールでは全く採掘できない。
- 追加: マントルディメンションのブロックの破壊時に、ツールの耐久値へ100倍のダメージが入る。
- 追加: マントルディメンションのブロックの硬度は、元にした岩石の100倍。
  - 岩石と鉱石は焼き石の100倍、ゲートと建材はテラコッタの100倍。

ツール

- 追加: ブリッジマナイト、ワズレアイト、リングウッダイトのツールを、それぞれつるはし・斧・シャベル・クワ・剣の5種類追加。
  - いずれも、Tier5のツールが必要なブロックを回収できる。
...
- 追加: ブロック「卵ブロック」を追加。
  - クラフトで入手可能。
  - 落下ブロック。
  - シルクタッチがなければそのまま回収できない。
  - シルクタッチなしでの破壊時および落下による破壊時に確率でひよこが出現する。
- 追加: 飲料アイテム「黒蜜」を追加。
  - アタノールで入手可能。
...
- 追加: 素材アイテム「コーパル」を追加。
  - かまどで精錬8回分の燃料として使える。
- 追加: 鉱石ブロック「土硝石鉱石」を追加。
  - 地上世界全域のY48～127の土に低確率で生成。
  - サバンナではより高い密度で生成。
- 追加: 鉱石ブロック「ネザー磁鉄鉱鉱石」を追加。
  - ネザー全域のY10～117のネザーラックにまばらに生成。
- 追加: ブロック「レチナイトブロック」を追加。
  - クラフトで入手可能。
- 追加: ブロック「コーパルブロック」を追加。
  - クラフトで入手可能。
  - ビーコンの土台になる。
- 追加: 装飾ブロック「石膏ボードのハーフブロック」を追加。
  - クラフトで入手可能。
...
- 追加: ブロック「析出した硫黄」を追加。
  - ヒカリゴケと同様に、ブロックの面に貼り付く。
  - 玄武岩デルタの、溶岩の表面の近くにある玄武岩およびブラックストーンの面に生成。
  - 採掘にはツルハシが必要。
  - 破壊時に硫黄を1個ドロップし、幸運で増加。
- 追加: ブロック「硫気孔」を追加。
  - 見た目を持たず、硫黄の色をした大きめの煙を0.5秒おきに噴き出す。
  - 煙は昇りながら風を受けて横にたなびく。
  - 析出した硫黄の塊の中央にあたる、溶岩の直上に生成。
  - 採掘にはツルハシが必要。
  - シルクタッチがなければ回収できない。
...
- 追加: ブロック「石化した樹脂」を追加。
  - 琥珀色の原生林の地表のおよそ半分と、その下の最大20ブロック程度の深さまでに生成。
  - 採掘には石以上のツルハシが必要。
- 追加: 鉱石ブロック「石化した樹脂レチナイト鉱石」を追加。
  - 琥珀色の原生林のY48～127の石化した樹脂に低確率で生成。
- 追加: 鉱石ブロック「石化した樹脂妖精のプラスチック鉱石」を追加。
  - 琥珀色の原生林のY48～127の石化した樹脂に低確率で生成。
- 追加: 鉱石ブロック「石化した樹脂コーパル鉱石」を追加。
  - 琥珀色の原生林のY48～127の石化した樹脂に低確率で生成。
- 追加: 装飾ブロック「石化した樹脂レンガ」を追加。
  - クラフトおよび石切台で入手可能。
- 追加: 装飾ブロック「石化した樹脂レンガのハーフブロック」を追加。
  - クラフトおよび石切台で入手可能。
- 追加: 装飾ブロック「石化した樹脂レンガの階段」を追加。
  - クラフトおよび石切台で入手可能。

プラノキ

- 追加: 素材アイテム「プラノキの樹液」を追加。
  - 食べられる。
    - 食べると粘着採掘Iを5分間得る。
    - 満腹度の回復量はハイメヴィスカの樹液と同等。
  - かまどで精錬1回分の燃料として使える。
  - 樹液タグに属し、樹液を材料とする各種レシピに使用できる。
- 追加: ブロック「プラノキの原木」を追加。
  - 縦向きのものに剣を使うと、傷を付けられる。
- 追加: ブロック「プラノキの葉」を追加。
  - 破壊すると、稀にプラノキの苗木を落とす。
  - 破壊すると、稀にプラノキの樹液を落とす。
- 追加: ブロック「プラノキの苗木」を追加。
  - 植えて育てると、プラノキが育つ。
- 追加: ブロック「傷の付いたプラノキの原木」を追加。
  - 時間経過で滴るプラノキの原木に変化する。
  - シルクタッチがなければプラノキの原木として回収される。
- 追加: ブロック「滴るプラノキの原木」を追加。
  - 正面から樹液の雫を垂らす。
  - 右クリックでプラノキの樹液を収穫できる。
    - 収穫後、ブロックは傷の付いたプラノキの原木に戻る。
    - メインハンドに持っているツールの幸運エンチャントが乗る。
    - 3%の確率で、追加でプラノキの樹液が出る。
      - 幸運1につき確率が1%増える。
  - 回収時、シルクタッチが無い限りプラノキの原木に化ける。
  - 回収時、追加でプラノキの樹液が出る。
    - 幸運1につき入手数が0～1増える。
  - 回収時、1%の確率で追加でプラノキの樹液が出る。
    - 幸運1につき入手数が0～2増える。
- 追加: 木「プラノキ」を追加。
  - 大きい木と小さい木の2種類がある。
  - 生成時に、幹の一部が滴るプラノキの原木に置き換わる。
  - 琥珀色の原生林の石化した樹脂の上に、大きい木1本につき小さい木2本の割合で生成される。
- 追加: ブロック「プラノキの樹液ブロック」を追加。
  - プラノキの樹液9個から作れ、9個に戻せる。
...
- 追加: 食べ物アイテム「琥珀糖」を追加。
  - アタノールで入手可能。砂糖で作るシンプルな琥珀糖。
  - 食べると移動速度上昇Iが2分間付与される。
- 追加: 食べ物アイテム「ハイメヴィスカの樹液の琥珀糖」を追加。
  - アタノールで入手可能。ハイメヴィスカの樹液で作る琥珀糖。
  - 食べると経験値獲得Iが8秒間付与される。
- 追加: 食べ物アイテム「プラノキの樹液の琥珀糖」を追加。
  - アタノールで入手可能。プラノキの樹液で作る琥珀糖。
  - 食べると粘着採掘Iが30分間付与される。
- 追加: 食べ物アイテム「黒蜜の琥珀糖」を追加。
  - アタノールで入手可能。黒蜜で作る琥珀糖。
  - 食べると移動速度上昇IIが2分間付与される。
- 追加: 食べ物アイテム「月のしずくの琥珀糖」を追加。
  - アタノールで入手可能。月のしずくで作る琥珀糖。
  - 食べると暗視Iが30分10秒間付与される。
- 追加: 食べ物アイテム「幻想の雫の琥珀糖」を追加。
  - アタノールで入手可能。幻想の雫で作る琥珀糖。
  - 食べると再生能力IIが30秒間、幸運IIが30分間付与される。
- 追加: 装飾ブロック「琥珀糖ブロック」6種類を追加。
  - クラフトおよびアタノールで入手可能。
  - それぞれ対応する琥珀糖9個と相互変換できる。
  - 氷のように半透明で、向こう側が透けて見える。
- 追加: 琥珀糖6種類は、その材料のパッシブスキル条件を満たす。

ハイメヴィスカ

- 変更: ハイメヴィスカの樹液と妖精のリキュールの経験値獲得の効果時間を、それぞれ2秒と16秒に延長。
  - 経験値の合計獲得量は、どちらも変更前と変わらない。
- 追加: 滴るハイメヴィスカの原木の使用時に粘着採掘が適用されるように変更。
- 追加: ハイメヴィスカの原木を剣で加工した際に、スカルクセンサーが反応するようになった。
- 追加: ハイメヴィスカの涙の燃料値を精錬8回分に上昇。
- 追加: ブロック「ハイメヴィスカの樹液ブロック」を追加。
  - ハイメヴィスカの樹液9個から作れ、9個に戻せる。
  - 上を移動する際の移動速度と跳躍力が下がる。
  - 側面を滑り降りる際の落下速度が抑えられ、落下ダメージが軽減される。
- 修正: 妖精の工場がフォリアを回収する際に、妖精の建物に加え、ハイメヴィスカの原木も辿るようになった。
  - これにより、妖精の建物からハイメヴィスカの原木を経由した先の葉からも、フォリアを回収できるようになった。
- 修正: 妖精の建物が、原木として扱われなくなった。
- 修正: チャージされていないハイメヴィスカの葉が枯れ落ちる際に、ドロップが2回発生するのを修正。
- 修正: ハイメヴィスカの感圧板が地図に表示されないのを修正。
...
- 表示: ハイメヴィスカの涙の日本語版ポエムを修正。
- 表示: 涙のシャベルの日本語版ポエムを修正。

レシピ

- 変更: 生命の水のアタノールレシピの材料を卵から卵ブロックに変更。
- 変更: 生命の水のアタノールレシピの材料を水ボトルからエタノールに変更。
- 変更: ラム酒のレシピをアタノールを使ったものに変更。
- 変更: パンのアタノールレシピの生産量を2個から1個に減少。
- 追加: 発酵した黒蜜入りバケツから固形燃料を作るアタノールレシピを追加。
- 追加: 荒い土から土と硝石を作るアタノールレシピを追加。
- 追加: 石化した樹脂から土とプラノキの樹液とレチナイトとコーパルを作るアタノールレシピを追加。
- 追加: 各種レシピの材料をハイメヴィスカの樹液から樹液タグに変更。

ツール

- 変更: 幻想のツルハシ・幻想のシャベル・幻想の剣の耐久値自然回復の速度を半分に減少。
...
- 追加: コーパルの基本ツール5種を追加。
  - クラフトで入手可能。
  - ツルハシ・斧・シャベル・クワは粘着採掘を持つ。
  - 剣は収集を持つ。
- 追加: ラピスラズリ製のツール5種を追加。
- 追加: 幻想のツルハシ・幻想のシャベル・幻想の剣に幸運IIと幸運強化Iを追加。
- 表示: ビルダーズロッドの使用時、逆の手に設置可能なブロックがない場合に、その旨のメッセージが表示されるようになった。

フェアリークエスト

- 変更: 「Re: Re: Re: パネッツァ氏の違法薬物所持の疑い」の入出力を紙から名札に変更。
- 追加: 「妖精の倫理について考える市民の会」の生産量を2倍に増加。
- 追加: 全画面版フェアリークエストカードに「ゲームに戻る」ボタンと「戻る」ボタンを追加。
- 表示: 全画面版フェアリークエストカードのレイアウトが変わった。
- 表示: 「Re: Re: Re: パネッツァ氏の違法薬物所持の疑い」のアイコンを名札に変更。
- 表示: 「Re: Re: Re: パネッツァ氏の違法薬物所持の疑い」の英語版の本文を修正。
- 表示: 「妖精の倫理について考える市民の会」の本文の各行の間に空行を追加。

魔法植物

- 変更: 特性「棘のある葉」の特性条件「低湿度」を削除。
- 変更: 特性「食虫植物」の特性条件「日照環境」を削除。
- 変更: ヴェロペダとサラセニアの接触ダメージを特性の効果によるものに変更。
- 追加: 特性効果「棘ダメージ」を追加。
  - 株に接触したエンティティに棘属性のダメージを与える。
  - 効果値がそのままダメージ量となる。
- 追加: 特性効果「捕食ダメージ」を追加。
  - 株に接触したエンティティに食虫植物による棘属性のダメージを与える。
  - ダメージ量の決まりかたは棘ダメージと同様である。
- 追加: 棘ダメージと捕食ダメージの両方を持つ株は、それらを合算した1回のダメージを与える。
  - このとき、ダメージタイプは捕食ダメージのものとなる。
- 追加: 特性「棘のある葉」に棘ダメージを追加。
- 追加: 特性「棘のある葉」の成長ブーストを0.2から0.5に上昇。
- 追加: 特性「食虫植物」に捕食ダメージを追加。
- 追加: 特性「食虫植物」の栄養値を0.2から0.5に上昇。
- 表示: 棘ダメージの殺害メッセージを追加。

地形生成

- 変更: ハイメヴィスカの葉が、幹から螺旋を描くように伸びる枝の先に付くようになった。
- 変更: 平原・森林バイオームでのハイメヴィスカの生成頻度を半分に減少。
  - 減らした分は小型のハイメヴィスカの生成に割り当てられた。
- 追加: ハイメヴィスカの幹が2x2の太さになった。
  - 苗木を2x2に並べて植えると、これが育つ。
- 追加: 平原・森林バイオームの草地に鳥の巣を追加。
  - 麦俵と卵ブロックで構成された3×3の構造物。
- 追加: 過去を見つめる都市に見つめる壁を追加。
  - 東西もしくは南北に直線状に伸びる、守護の耐霊石レンガの壁。
  - 中央付近の2か所に模様入りの守護の耐霊石が埋め込まれている。
- 追加: バイオーム「琥珀色の原生林」を追加。
  - 多湿バイオームであり、魔法植物の「高湿度」特性条件が成立する。
  - 沼地と同様に、火の延焼が起こりにくい。
  - 土妖精のプラスチック鉱石が生成される。
  - シダ・大きなシダ・キノコは生成されない。
- 追加: 琥珀色の原生林の地面にプラノキの倒木を追加。
  - 折れ残った切り株と、そこから水平に横たわる5～9ブロックの幹で構成される。
- 追加: ハイメヴィスカにツタが生えるようになった。
- 追加: 小型のハイメヴィスカを追加。
  - 幹が1x1の太さで、枝を持たない。
  - 葉が幹に沿って螺旋を描くように付く。
  - 苗木を1個だけ植えると、これが育つ。
  - 妖精の森、および平原・森林バイオームに生成される。
- 追加: 砂岩硝石鉱石の生成頻度を2倍に増加。
- 修正: 土妖精のプラスチック鉱石が、草ブロックやポドゾルなどではなく、土ブロックのみに生成されるようになった。
- 修正: 妖精の樹海・過去を見つめる都市の地表のブロックの分布をわずかに変更。
- 修正: 妖精の樹海の地表に、粗い土とポドゾルが生成されない問題を修正。

ユーザーインターフェース

- 追加: レシピビューワーの「フェアリークエスト」タブで、依頼文を閲覧できるようになった。
  - クエスト名をクリックすると、依頼文が全画面で表示される。
- 修正: 共通機械レシピのレシピビューワーの出力にレシピリマインダーが含まれないのを修正。
- 修正: 範囲採掘やビルダーズロッドの形状オーバーレイが、ワールド原点から遠く離れた場所で細かく揺れるのを修正。
- 修正: レシピビューワーでアタノール・オーラ反射炉・醸造樽の所要時間が中央揃えにならないのを修正。
- 表示: 各種食べ物アイテムのツールチップに、付与されるステータス効果が表示されるようになった。
  - これに伴い、その効果を述べていた説明文は削除された。
- 表示: がれきの生成条件をレシピビューワーで確認できるようになった。
- 表示: レシピビューワーの「収穫」タブのツールチップに、そのドロップ品が生じる判定の名前が表示されるようになった。
- 表示: 範囲採掘やビルダーズロッドの形状オーバーレイが、F1によるHUDの非表示中を含め、バニラのブロックの輪郭と同じ条件で隠れるようになった。
- 表示: 大鎌でブロックを狙っている間、収穫範囲に緑色の形状オーバーレイが表示されるようになった。
  - 範囲採掘による破壊範囲の形状オーバーレイとは、色で見分けられる。

テクスチャとポエム

- 追加: 土妖精のプラスチック鉱石にポエムを追加。
- 表示: 妖精のプラスチックのシャベルの日本語版ポエムを修正。
- 表示: 妖精のプラスチックの英語版ポエムを修正。
- 表示: 蒼天石鉱石・深層蒼天石鉱石のポエムの tier を 1 → 2 に変更。
- 表示: ツールおよび特性条件の名称に含まれる「つるはし」を、バニラに合わせて「ツルハシ」に修正。
- 表示: 土妖精のプラスチック鉱石のテクスチャを変更。
- 表示: 各種硝石鉱石のテクスチャを変更。
...
- 変更: 紅天石ツールのエンチャント適正を低下。
- 変更: ステータス効果「経験値獲得」は、1秒間にレベルと同じ量の経験値を得るようになった。
- 変更: 月のしずくの暗視の効果時間を5分10秒に延長。
- 変更: 幻想の雫を食べたときの再生能力Iの効果時間を30秒に短縮し、代わりに幸運Iが5分間付与されるようになった。
- 変更: オーラ反射炉のホッパー投入面を変更。
  - 燃料スロットには、正面と背面から投入できる。
  - 中央の入力スロットには、上面から投入できる。
  - 中央の入力スロットへ背面および下面から投入することはできなくなった。
  - 上面から燃料を投入すると、燃料スロットではなく中央の入力スロットに入る。
- 追加: 妖精のプラスチックを入手したときの進捗を追加。
- 追加: 磁鉄鉱鉱石・深層磁鉄鉱鉱石・磁鉄鉱ブロックの採掘時に、所持ツールによらず粘着採掘が発動するようになった。
- 追加: 各種鉱石ブロックのアイテムに鉱石辞書系のタグを追加。
- 追加: フラクタルウィスプがオーラ反射炉の燃料にできるようになった。
- 修正: 大鎌の範囲収穫が、ブロックに遮られた先には届かなくなった。
- 修正: アタノールで水入り瓶を消費した際にガラス瓶が返却されないのを修正。
- 修正: 採掘した際に入手する妖精に粘着採掘が適用されない問題を修正。
- 修正: 撃破した際に入手する妖精に粘着採掘が適用されない問題を修正。

クリエイティブ専用

- 追加: クリエイティブタブ「ツール」を追加。
  - MODの主要なクリエイティブタブにあった各種ツールが、こちらに移動。
- 追加: クリエイティブ専用ツール「アンチアカシックロッド」を追加。
  - 使用すると、狙った面と地続きになっている同じ状態のブロックを、まとめて破壊できる。
  - ビルダーズロッドとは逆に、狙った面の奥側のブロックに作用する。
  - 手前が塞がっていない面のみを、面に沿って16ブロックの範囲まで辿る。
  - 側方範囲採掘によって範囲が拡張する。
...
- 追加: 以下を追加。
  ...
  - 鉱石ブロック「硫黄鉱石」「深層硫黄鉱石」
    - Y-64～0に鉱石が生成。
  - 鉱石ブロック「硝石鉱石」「深層硝石鉱石」「砂岩硝石鉱石」
    - 砂漠およびサバンナのY48～128および鍾乳洞に鉱石が生成。
  - 鉱石ブロック「土妖精のプラスチック鉱石」
    - 妖精の樹海のY48～128の空気に面していない土に低確率で生成。
  - 素材ブロック「ミラージュの葉ブロック」
    - クラフトで入手可能。
    - 素手で破壊すると棘属性ダメージを受ける。
  - 素材アイテム「灰」
  ...
  - 建材ブロック「妖精のセラミックレンガ」
    - クラフトで入手可能。
  - 建材ブロック「妖精のセラミックレンガのハーフブロック」
    - クラフトで入手可能。
  - 建材ブロック「妖精のセラミックレンガの階段」
    - クラフトで入手可能。
- 追加: 妖精のプラスチックがサバイバルで入手可能になった。
- 追加: 夜のかけらがサバイバルで入手可能になった。

レシピ

- 変更: スフィアベースのレシピをアタノールを使ったものに変更。
- 変更: 守護の耐霊石の丸石のレシピをアタノールを使ったものに変更。
- 追加: 硝石・石炭／木炭・硫黄から火薬を作る作業台レシピを追加。
- 追加: 固形燃料のアタノールレシピを追加。
- 追加: 夜のかけらのアタノールレシピを追加。
- 追加: 固形燃料からTNTを作る作業台レシピを追加。
- 追加: パンのアタノールレシピを追加。
- 追加: 板材または木の棒から灰を作るアタノールレシピを追加。
- 追加: ハイメヴィスカの樹液から砂糖を作るアタノールレシピを追加。
- 追加: 紅天石を分解するアタノールレシピを追加。
- 追加: 各種守護の耐霊石を分解するアタノールレシピを追加。
- 追加: 「妖精変成レシピ」に類するアタノールレシピ群を追加。
  - 妖精を触媒にし、生命の水とその他の材料から生物系アイテムを作る。
...
- 変更: 加工機械ブロック全般のホッパー入出力面を変更。
- 修正: 加工機械ブロックのホッパー入出力の向きの判定を修正。
- 修正: 加工機械ブロックの入力スロットが任意の順序で受け付けるようになった。
- 修正: オーラ反射炉の燃料をレシピビューワーで入力から無視するように修正。

フェアリークエスト

- 変更: 全体的にレシピの入出力を変更。
- 変更: 一部のフェアリークエストの加工時間を0.5秒に短縮。
- 追加: 以下のフェアリークエストを追加。
  - 「Re: Re: Re: パネッツァ氏の違法薬物所持の疑い」
  - 「シャルパの夜 ～夜空を彩るレジンの星～」
  - 「妖精の倫理について考える市民の会」
- 表示: 「非常事態」のアイコンを紙に変更。
- 表示: レシピビューワーでタイトルテキストにツールチップを追加。

魔法植物

- 変更: ヴェロペダ・サラセニアに棘属性接触ダメージを追加。
- 追加: 魔法植物の収穫時にメインハンドの粘着採掘が適用されるように変更。
- 追加: 大鎌の範囲収穫部分に粘着採掘が適用されるように変更。
- 表示: 魔法植物の収穫時のパーティクルを追加。
- 表示: アカーシャによる生命設計の針のアイコンに特性オーバーレイを追加。

地形生成

- 追加: フェアリーリング（環状のミラージュの花）に以下の地形生成物を追加。
  - ランダムな妖精の鱗粉
  - 中央の地下に蒼天石鉱石および蒼天石ブロック
- 追加: 妖精の樹海にスケルトンの高架スポーナーを追加。

その他

- 追加: ラム酒の燃料値を精錬12回分に上昇。
- 追加: 各種蒼天石・蒼天石タイル系ブロックに局所真空崩壊耐性を追加。
- 追加: ルミナイトがオーラ反射炉の燃料にできるようになった。
- 修正: いくつかのブロックの延焼速度を変更。
- 修正: 混沌のキューブの初期化処理のバグを修正。

## 27.30.0

**バイオーム配置の方式の変更により、新旧バージョンの地形でバイオーム境界が一貫しない可能性あり。**

過去を見つめる都市

- 追加: バイオーム「過去を見つめる都市」を追加。
  - バニラのサバンナ・荒れ地の一部を置換して発生する。
  - 黄土色の植生。
  - 地面の一部が耐霊石系のブロックで出来ている。
  - 混沌のキューブが自然にスポーンする。
  - 混沌のキューブが自発的に敵対する。
- 追加: 過去を見つめる都市に以下の地形生成物を追加。
  - ミラジディアンの街灯
  - 過去を見つめる都市の小さな遺構
  - 過去を見つめる都市の極小の遺構
  - 紅天市街のオベリスク
...
- 修正: バイオーム配置の方式を根本的に修正。
  - 妖精の森はバニラの森林系バイオームの一部を置換して発生する。
  - 妖精の樹海はバニラのタイガ系バイオームの一部を置換して発生する。
- 修正: MOD産のバイオームでいくつかの地形生成物が出現しない問題を修正。

ツール

- 追加: ビルダーズロッドの進捗を追加。
- 修正: ビルダーズロッド系アイテムのエンチャント判定を修正。
  - 通常採掘系エンチャント全般が付与可能になった。
- 表示: 理天石のクワの日本語版ポエムを修正。

その他

- 追加: オーラ反射炉の燃料のレシピビューワータブを追加。
- 修正: ゴールドプロミナリア生成時に金インゴットを消費するように修正。
- 修正: ソウルストリームのフィルタのクリアボタンの表示を修正。
- 修正: オーラ反射炉を素手で壊せないように修正。
- 表示: 各種ダメージタイプの殺害メッセージを修正。

クリエイティブ専用

- 追加: デバッグアイテム「Dump Biome Features」を追加。
- 修正: 以下のエンチャントの本がはさみ等に付与可能になった。
  - 粘着採掘
  - 前方範囲採掘
  - 側方範囲採掘
  - 後方範囲採掘
...
- 変更: 日照環境は天体の表示されるディメンションでのみ有効となった。
- 追加: 特性条件に「晴れ」および「夜」を追加。
- 追加: 特性「月光合成」を追加。
  - 天体の光が当たる場所で、降水中でないとき、夜間に栄養値を与える。
- 追加: メルルルリアの所持特性に月光合成を追加。

その他

- 追加: ビルダーズロッドが側方範囲採掘によって範囲が拡張するようになった。
- 修正: 範囲採掘系エンチャントの巻き込み分で耐久値が減らないバグを修正。
- 表示: オブシディアーニャの名称を「黒曜石精オブシディアーニャ」に変更。
...
- 追加: 以下のアイテムを追加。
  ...
  - 「理天石の大鎌」
  - 「ビルダーズロッド」
    - 使用時、逆の手に持ったブロックを使用面に合わせて大量に設置する。
    - 手のブロックを使い切るとインベントリから補充する。
- 修正: 範囲採掘系エンチャントで硬めのブロックが残る問題を修正。
...
- 変更: 以下のツール効果を削除し、代わりに同等のエンチャントに変更。
  - 範囲採掘
  - 木全体を伐採
  - 鉱石全体を採掘
- 変更: 範囲採掘時に採掘対象ブロックの硬度の合計を参照するように変更。
- 追加: 以下のエンチャントを追加。
  - 「前方範囲採掘」: 採掘面の前方に向かって直方体状に採掘範囲を広げます。
  - 「側方範囲採掘」: 採掘面の側方に向かって直方体状に採掘範囲を広げます。
  - 「後方範囲採掘」: 採掘面の後方に向かって直方体状に採掘範囲を広げます。
  - 「一括伐採」: 一つながりの幹と葉を一括で破壊します。
  - 「一括採掘」: 一つながりの鉱石を一括で破壊します。
  - 「範囲採掘加速」: (2^Lv)個を同時に採掘しても採掘時間が倍加しません。
- 追加: 範囲採掘対象のプレビュー表示を追加。
- 修正: 範囲採掘系効果の発動時、演出が出ない問題を修正。

ツール

- 変更: いくつかのツールの性能を調整。
  - 自動発動するエンチャントに範囲採掘加速を追加。
  - 採掘速度を変更。
  - エンチャント適正を変更。
...
- 変更: 一部の特性効果の計算式を下方修正。
- 変更: メルルルリアの月のしずくを葉面判定から希少品判定に変更。
- 変更: 肉厚の葉の効果を葉面生成に変更。
- 変更: 高嶺の花の効果を希少品生成に変更。
- 変更: 砂漠の宝石の効果を特殊品生成に変更。
- 追加: 条件が設定された特性の効果を上方修正。

## 23.26.0-beta.1

- 修正: 死亡中のプレイヤーは妖精の夢を入手できないように修正。
- 修正: 混沌のキューブがスペクテイターのプレイヤーを見失うように修正。
...
- 修正: 混沌のツルハシのツールチップに対応ブロックの表記を追加。
- 表示: 特性GUIの表示を微調整。
...
- 追加: ネガティブビット機能を追加。
  ...
  - コマンド「 `/mf24kuc trait negative 特性 [ビット]` 」で指定する。
    - ビット省略時、全ビットをネガティブ指定。
    - 解除するにはビットを0とする。
- 追加: コマンド「 `/mf24ku dream give (all|モチーフ)` 」を追加。
  - 指定の妖精の夢をプレイヤーに付与する。
- 追加: コマンド「 `/mf24ku dream remove (all|モチーフ)` 」を追加。
  - 指定の妖精の夢をプレイヤーから削除する。
...
- 追加: ルミナリア科植物の基本種子生成率を20%から100%に上昇。
...
- 変更: 範囲採掘効果の仕様を変更。
  ...
  - 混沌のツルハシおよび妖精の木の涙製ツールの範囲採掘の範囲を変更。
...
- 追加: ユーティリティブロック「妖精の像の泉」を追加。
  - 100フェアリージュエルを使用して抽選ができる。
- 追加: ユーティリティブロック「妖精の像」を追加。
...
- 追加: ミラジウムのツルハシ・紅天石のツルハシに鉱石の一括採掘機能を追加。
- 追加: 以下のアイテムに自己修繕機能を追加。
  - フェアリークリスタルのツルハシ
  - ミラジウムのツルハシ
  - ミラジウムの斧
- 追加: 蒼天石のツルハシがシルクタッチを持つようになった。
  - エンチャントとは別枠で発動するため、更にエンチャントを付与できる。
- 追加: 混沌のツルハシが砂系ブロックを高速に掘れるようになった。
- 追加: 混沌のツルハシに範囲採掘機能を追加。
- 追加: 範囲採掘系効果において、硬度が0のブロックでは耐久度が減らないようになった。
- 表示: フェアリークリスタルのツルハシの英語版のポエムを変更。
...
- 追加: 素材「ミラージュの花粉」を追加。
  - v0時点において、用途はなし。
- 追加: ミラージュの花粉の圧縮レベルの異なるバリエーションを追加。
  - 「小さなミラージュの花粉」: 1/9倍
  - 「高純度ミラージュの花粉」: 9倍
  - 「特選高純度ミラージュの花粉」: 9×9倍
  - 「厳選高純度ミラージュの花粉」: 9×9×9倍
  - 「激甚高純度ミラージュの花粉」: 9×9×9×9倍
  - 「極超高純度ミラージュの花粉」: 9×9×9×9×9倍
- 追加: 素材「ミラージュの葉」を追加。
  - コンポスターに投入可能。
- 追加: 素材「ミラージュの茎」を追加。
  - ミラージュの茎からクラフトできる。
...
- 追加: 以下の特性条件を追加。
  ...
  - 地面が結晶質であるとき: 以下のリストに従って効果値に倍率がかかる。
    ...
    - 銅ブロック: 0.2
    - その他: 0.0
  - 地面が硬いとき: 地面がツルハシで採掘可能なとき、硬度が0～4にかけて0～2の倍率がかかる。
