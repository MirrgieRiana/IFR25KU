...
enum class DebrisCard(
    ...
    val biomeCondition: BiomeCondition,
    val extraPlacementModifier: PlacementModifiersScope.() -> List<PlacementModifier> = { emptyList() },
) {
    STICK("stick", 32, 2..6, { Items.STICK.createItemStack() }, BiomeCondition.BiomeTag(BiomeTags.IS_OVERWORLD)),
    STICK_DENSE("stick_dense", 32 / 8, 2..6, { Items.STICK.createItemStack() }, BiomeCondition.BiomeTag(BiomeTags.IS_FOREST)),
    BONE("bone", 64, 2..6, { Items.BONE.createItemStack() }, BiomeCondition.BiomeTag(BiomeTags.IS_OVERWORLD)),
    STRING("string", 64, 2..6, { Items.STRING.createItemStack() }, BiomeCondition.BiomeTag(BiomeTags.IS_OVERWORLD)),
    FLINT("flint", 64, 2..6, { Items.FLINT.createItemStack() }, BiomeCondition.BiomeTag(BiomeTags.IS_OVERWORLD)),
    RAW_IRON("raw_iron", 128, 2..6, { Items.RAW_IRON.createItemStack() }, BiomeCondition.BiomeTag(BiomeTags.IS_OVERWORLD)),
    RAW_IRON_DENSE("raw_iron_dense", 128 / 2, 8..24, { Items.RAW_IRON.createItemStack() }, BiomeCondition.BiomeTag(BiomeTags.IS_MOUNTAIN)),
    RAW_COPPER("raw_copper", 128, 2..6, { Items.RAW_COPPER.createItemStack() }, BiomeCondition.BiomeTag(BiomeTags.IS_OVERWORLD)),
    RAW_COPPER_DENSE("raw_copper_dense", 128 / 2, 8..24, { Items.RAW_COPPER.createItemStack() }, BiomeCondition.BiomeTag(BiomeTags.IS_MOUNTAIN)),
    XARPITE("xarpite", 128, 2..6, { MaterialCard.XARPITE.item().createItemStack() }, BiomeCondition.BiomeTag(BiomeTags.IS_OVERWORLD)),
    ...
    DebrisCard.entries.forEach { card ->
        DEBRIS_FEATURE.generator(card.identifier) {
            registerConfiguredFeature { DebrisFeature.Config(UniformInt.of(card.count.first, card.count.last), card.itemStackGetter()) }.generator {
                ...
                }.placeWhenVegetalDecoration { +card.biomeCondition }
            }
        }
    }
}
