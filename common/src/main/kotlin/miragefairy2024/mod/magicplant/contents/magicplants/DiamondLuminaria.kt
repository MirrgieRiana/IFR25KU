...
object DiamondLuminariaCard : AbstractLuminariaCard<DiamondLuminariaBlock>() {
    ...
    override val drops = listOf({ HarvestNotation.Crop(MaterialCard.LUMINITE.item().createItemStack(), TraitEffectKeyCard.RARE_PRODUCTION.traitEffectKey.name) }, { HarvestNotation.Crop(Items.DIAMOND.createItemStack(), TraitEffectKeyCard.SPECIAL_PRODUCTION.traitEffectKey.name) })
    ...
    override fun init() {
        ...
        Feature.FLOWER.generator(blockIdentifier) {
            registerConfiguredFeature("cluster") { RandomPatchConfiguration(1, 0, 0, placer) }.generator {
                registerPlacedFeature("cluster") { per(128) + flower(square, surface) }.placeWhenVegetalDecoration { (+ConventionalBiomeTags.IS_SNOWY + +ConventionalBiomeTags.IS_ICY) * defaultTraits }
            }
        }
    }
}
