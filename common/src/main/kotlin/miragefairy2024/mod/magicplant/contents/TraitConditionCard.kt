...
enum class TraitConditionCard(
    ...
    enName: String,
    jaName: String,
    function: (TraitConditionContext) -> Double,
) {
    FLOOR_MOISTURE(
        "floor_moisture", Emoji.FLOOR_MOISTURE, "Floor Moisture", "湿った地面",
        { it.world.getMoisture(it.blockPos.below()) },
    ),
    FLOOR_CRYSTAL_ERG(
        "floor_crystal_erg", Emoji.FLOOR_CRYSTAL_ERG, "Floor Crystal Erg", "鉱物質の地面",
        { it.world.getCrystalErg(it.blockPos.below()) },
    ),
    FLOOR_HARDNESS(
        "floor_hardness", Emoji.FLOOR_HARDNESS, "Floor Hardness", "硬い地面",
        { getFloorHardness(it.world, it.blockPos) },
    ),
    LIGHT(
        "light", Emoji.LIGHT, "Light", "光",
        { (it.world.lightProxy.getLightLevel(it.blockPos) - 8 atLeast 0) / 7.0 },
    ),
    DARKNESS(
        "darkness", Emoji.DARKNESS, "Darkness", "闇",
        { ((15 - it.world.lightProxy.getLightLevel(it.blockPos)) - 8 atLeast 0) / 7.0 },
    ),
    LOW_TEMPERATURE(
        "low_temperature", Emoji.LOW_TEMPERATURE, "Low Temperature", "低い気温",
        { if (it.world.getBiome(it.blockPos).temperatureCategory == TemperatureCategory.LOW) 1.0 else 0.0 },
    ),
    MEDIUM_TEMPERATURE(
        "medium_temperature", Emoji.MEDIUM_TEMPERATURE, "Medium Temperature", "普通の気温",
        { if (it.world.getBiome(it.blockPos).temperatureCategory == TemperatureCategory.MEDIUM) 1.0 else 0.0 },
    ),
    HIGH_TEMPERATURE(
        "high_temperature", Emoji.HIGH_TEMPERATURE, "High Temperature", "高い気温",
        { if (it.world.getBiome(it.blockPos).temperatureCategory == TemperatureCategory.HIGH) 1.0 else 0.0 },
    ),
    LOW_HUMIDITY(
        "low_humidity", Emoji.LOW_HUMIDITY, "Low Humidity", "低い湿度",
        { if (it.world.getBiome(it.blockPos).humidityCategory == HumidityCategory.LOW) 1.0 else 0.0 },
    ),
    MEDIUM_HUMIDITY(
        "medium_humidity", Emoji.MEDIUM_HUMIDITY, "Medium Humidity", "普通の湿度",
        { if (it.world.getBiome(it.blockPos).humidityCategory == HumidityCategory.MEDIUM) 1.0 else 0.0 },
    ),
    HIGH_HUMIDITY(
        "high_humidity", Emoji.HIGH_HUMIDITY, "High Humidity", "高い湿度",
        { if (it.world.getBiome(it.blockPos).humidityCategory == HumidityCategory.HIGH) 1.0 else 0.0 },
    ),
    OUTDOOR(
        "outdoor", Emoji.OUTDOOR, "Outdoor", "屋外",
        { if (it.blockPos.y >= it.world.getHeightmapPos(Heightmap.Types.MOTION_BLOCKING, it.blockPos).y) 1.0 else 0.0 },
    ),
    SUNSHINE_ENVIRONMENT(
        "sunshine_environment", Emoji.OUTDOOR, "Sunshine Environment", "日照環境",
        { it.world.getNaturalDimensionFactor() * it.world.getPermanentSkyLightLevelFactor(it.blockPos) },
    ),
    NATURAL(
        "natural", Emoji.NATURAL, "Natural", "天然",
        { if (it.blockEntity?.isNatural() == true) 1.0 else 0.0 },
    ),
    HIGH_ALTITUDE(
        "high_altitude", Emoji.UP, "High Altitude", "高地",
        { it.world.getHighAltitudeFactor(it.blockPos) },
    ),
    LOW_ALTITUDE(
        "low_altitude", Emoji.DOWN, "Low Altitude", "低地",
        { it.world.getLowAltitudeFactor(it.blockPos) },
    ),
    ;
    ...
    val traitCondition = object : TraitCondition {
        ...
        override fun getFactor(context: TraitConditionContext) = function(context)
