...
enum class SimplePassiveSkillConditionCard(path: String, enName: String, jaName: String, private val function: (context: PassiveSkillContext) -> Boolean) : PassiveSkillCondition {
    OVERWORLD("overworld", "Overworld", "地上世界", { it.level.dimensionType().natural }),
    OUTDOOR("outdoor", "Outdoor", "屋外", { it.level.isOutdoor(it.blockPos) }),
    INDOOR("indoor", "Indoor", "屋内", { !it.level.isOutdoor(it.blockPos) }),
    SKY_VISIBLE("sky_visible", "Sky Visible", "空が見える", { it.level.canSeeSky(it.blockPos) }),
    FINE("fine", "Fine", "晴天", { it.level.getEnvironmentalWeatherAt(it.blockPos) == Biome.Precipitation.NONE }),
    RAINING("raining", "Raining", "雨天", { it.level.getEnvironmentalWeatherAt(it.blockPos) == Biome.Precipitation.RAIN }),
    THUNDERING("thundering", "Thundering", "雷雨", { it.level.getEnvironmentalWeatherAt(it.blockPos) != Biome.Precipitation.NONE && it.level.isThundering }),
    DAYTIME("daytime", "Daytime", "昼間", { it.level.isDaytime }),
    NIGHT("night", "Night", "夜間", { !it.level.isDaytime }),
    ...
    IN_NETHER("in_nether", "In Nether", "ネザー", { it.level.getBiome(it.blockPos) isIn ConventionalBiomeTags.IS_NETHER }),
    NOT_IN_NETHER("not_in_nether", "Not In Nether", "ネザー外", { it.level.getBiome(it.blockPos) isNotIn ConventionalBiomeTags.IS_NETHER }),

    PICKAXE("pickaxe", "Pickaxe", "ツルハシ", { it.player.mainHandItem isIn ItemTags.PICKAXES }),
