...
object SmallPlasticTreeFoliagePlacerCard {
    val identifier = MirageFairy2024.identifier("small_plastic_tree")
    private val codec: MapCodec<SmallPlasticTreeFoliagePlacer> = RecordCodecBuilder.mapCodec { instance ->
        instance.group(
        ...
        ).apply(instance, ::SmallPlasticTreeFoliagePlacer)
    }
    val type: FoliagePlacerType<SmallPlasticTreeFoliagePlacer> = FoliagePlacerType(codec)
}

class SmallPlasticTreeFoliagePlacer(radius: IntProvider, offset: IntProvider, val height: Int) : FoliagePlacer(radius, offset) {
    val radiusProvider: IntProvider get() = this.radius
    val offsetProvider: IntProvider get() = this.offset

    override fun type() = SmallPlasticTreeFoliagePlacerCard.type
