...
abstract class TreeBlockCard(val configuration: TreeBlockConfiguration) {
    companion object {
        ...
        ).tag(BlockTags.LEAVES, ItemTags.LEAVES).tag(BlockTags.MINEABLE_WITH_HOE).let { TreeChargeableLeavesBlockCard(it) { SAPLING } }
        ...
        ).tag(BlockTags.LEAVES, ItemTags.LEAVES).tag(BlockTags.MINEABLE_WITH_HOE).let { TreeLeavesBlockCard(it) { SAPLING } }
    }
