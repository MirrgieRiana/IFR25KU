...
fun initParticleClientModule() {
    ...
    ParticleFactoryRegistry.getInstance().register(ParticleTypeCard.DRIPPING_SAP.particleType) { spriteProvider -> ParticleProvider { _, world, x, y, z, _, _, _ -> SapParticle.Dripping(world, x, y, z, spriteProvider, ParticleTypeCard.FALLING_SAP.particleType) } }
    ParticleFactoryRegistry.getInstance().register(ParticleTypeCard.FALLING_SAP.particleType) { spriteProvider -> ParticleProvider { _, world, x, y, z, _, _, _ -> SapParticle.Falling(world, x, y, z, spriteProvider, ParticleTypeCard.LANDING_SAP.particleType) } }
    ParticleFactoryRegistry.getInstance().register(ParticleTypeCard.LANDING_SAP.particleType) { spriteProvider -> ParticleProvider { _, world, x, y, z, _, _, _ -> SapParticle.Landing(world, x, y, z, spriteProvider) } }
